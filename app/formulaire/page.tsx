...
export default function FormulaireEnlevement() {
  ...
  const loadRecaptchaScriptReady = async (): Promise<void> => {
    if (!siteKey) return;
    ...
    await new Promise<void>((resolve) => {
      ...
      script.onload = () => {
        ...
        if (ww.grecaptcha && ww.grecaptcha.ready) {
          ww.grecaptcha.ready(() => resolve());
        } else {
          resolve();
        }
      };
  ...
  const getRecaptchaToken = async (action: string): Promise<string | null> => {
    try {
      ...
      await loadRecaptchaScriptReady();
      const w = window as any;
      if (w.grecaptcha && typeof w.grecaptcha.execute === "function") {
        const token: string = await w.grecaptcha.execute(siteKey, { action });
        return token || null;
      }
      return null;
    } catch {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const provided = !!formData.email && formData.email.trim().length > 0;
      setEmailProvided(provided);
      if (provided) setSubmittedEmail(formData.email.trim());
      const recaptchaToken = await getRecaptchaToken("form_submit");
      // POST vers l'API pour persistance DB et envoi e-mail
      ...
      if (!res.ok) throw new Error("Échec de l'envoi de la demande");
      ...
      <div className="min-h-screen bg-gray-50 flex items-center justify-center p-4">
        <Card className="max-w-md w-full text-center" role="status" aria-live="polite">
          <CardContent className="p-8">
            ...
            {emailProvided && (
              <p className="text-gray-600 mb-6">
                Une confirmation a été envoyée {submittedEmail ? `à ${submittedEmail}` : "à l'adresse fournie"}.
              </p>
            )}
                ...
                <CardContent className="space-y-4">
                  <div className="flex items-center space-x-3">
                    ...
                    <div>
                      ...
                      <a href="tel:+33659128819" className="text-gray-600 hover:text-yellow-600" aria-label="Appeler au +33 6 59 12 88 19">
                        +33 6 59 12 88 19
                      </a>
                    </div>
                  </div>
              ...
              <Card className="bg-yellow-50 border-yellow-200">
                <CardContent className="p-6">
                  ...
                  <ul className="space-y-2 text-sm">
                    ...
                    <li className="flex items-center space-x-2">
                      <CheckCircle className="w-4 h-4 text-green-500" aria-hidden="true" />
                      <span className="text-gray-700">Certificat de destruction fourni</span>
                    </li>
                    <li className="flex items-center space-x-2">
                      <CheckCircle className="w-4 h-4 text-green-500" aria-hidden="true" />
                      <span className="text-gray-700">Service agréé VHU</span>
                    </li>
                  </ul>
                </CardContent>
              </Card>
