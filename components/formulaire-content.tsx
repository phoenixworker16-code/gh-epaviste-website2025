...
import { CheckCircle, Phone, Mail, MapPin, User, Car } from "lucide-react";
import Link from "next/link";
import BreadcrumbNav from "@/components/breadcrumb-nav";
import ScrollToTop from "@/components/scroll-to-top";

export default function FormulaireContent() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        throw new Error(errorData.error || "Échec de l'envoi de la demande.");
      }
    } catch (error: any) {
      alert(error.message || "Une erreur est survenue lors de l'envoi de votre demande. Veuillez réessayer.");
    } finally {
        ...
        <Card className="max-w-md w-full text-center" role="status" aria-live="polite">
          <CardContent className="p-8">
            ...
            <div className="text-gray-600 mb-6 space-y-1">
              <p className="font-semibold text-green-600">Votre demande a été envoyée avec succès.</p>
                  ...
                  <form onSubmit={handleSubmit} className="space-y-8">
                    ...
                    <div>
                      <Label htmlFor="message" className="text-black font-medium">
                        Message <span className="text-gray-400 font-normal text-sm">(facultatif)</span>
                      </Label>
                      <Textarea
                        id="message"
                        value={formData.message}
                        onChange={(e) => handleChange("message", e.target.value)}
                        className="mt-1 focus:border-yellow-500 focus:ring-yellow-500 min-h-[100px]"
                        placeholder="Décrivez votre demande (type de véhicule, situation particulière...)"
                        disabled={isSubmitting}
                      />
                    </div>
                ...
                <CardContent className="space-y-4">
                  <div className="flex items-center space-x-3">
                    ...
                    <div>
                      ...
                      <a href="tel:+33753120793" className="text-gray-600 hover:text-yellow-600" aria-label="Appeler au 07 53 12 07 93">
                        07 53 12 07 93
                      </a>
                    </div>
                  </div>
